...
/// Branch Target report mein branch par click karne par khulta hai — us
/// branch ke har din ka sale target, total sale aur achieve hua ya nahi.
class BranchTargetDetailDialog extends ConsumerStatefulWidget {
  final BranchTargetRow row;
  const BranchTargetDetailDialog({super.key, required this.row});

  static Future<void> show(BuildContext context, BranchTargetRow row) =>
      showDialog(context: context, builder: (_) => BranchTargetDetailDialog(row: row));

  @override
  ConsumerState<BranchTargetDetailDialog> createState() => _BranchTargetDetailDialogState();
}
...
class _DayLine {
  ...
  final bool isToday;
  final bool isFuture;
  const _DayLine(this.date, this.target, this.sale, this.isToday, this.isFuture);
  bool get achieved => !isFuture && target > 0 && sale >= target;
}

class _BranchTargetDetailDialogState extends ConsumerState<BranchTargetDetailDialog> {
  ...
  Future<void> _load() async {
    try {
      ...
      final targets =
          await ref.read(branchTargetDatasourceProvider).fetchFrom(id, DateTime(2000));
      ...
      setState(() {
        _lines = [
          for (final d in days)
            _DayLine(d, targets[d]!, sales[d] ?? 0, d == today, d.isAfter(today)),
        ];
        _loading = false;
      });
    } catch (e) {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Row(children: [
                Expanded(
                  child: Text('${widget.row.branchName} — Target Report',
              ...
              if (!_loading && _lines.isNotEmpty) ...[
                ...
                Wrap(spacing: 20, runSpacing: 6, children: [
                  _chip('Total Target', _amt(totalTarget), const Color(0xFF3E63DD)),
                  _chip('Total Sale (till today)', _amt(totalSale),
                      totalSale >= 0 ? _green : Colors.red),
                  _chip('Achieved Days', '$achieved / ${past.length}', _green),
                ]),
              ],
  ...
  DataRow _row(_DayLine l) {
    ...
    return DataRow(cells: [
      DataCell(Text('${_fmtDate(l.date)}  ${_weekdays[l.date.weekday - 1]}${l.isToday ? '  (Today)' : ''}',
          style: TextStyle(fontWeight: l.isToday ? FontWeight.w700 : FontWeight.w400))),
      DataCell(Text(_amt(l.target))),
      DataCell(Text(l.isFuture ? '—' : _amt(l.sale),
