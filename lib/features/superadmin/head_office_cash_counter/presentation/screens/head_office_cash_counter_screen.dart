...
import '../providers/head_office_cash_counter_provider.dart';
import '../providers/head_office_cash_counter_state.dart';

import 'package:safishoe_app/core/widget/app_icon.dart';
import 'package:safishoe_app/core/constants/app_icons.dart';
import 'package:safishoe_app/core/widget/text_field_icon.dart';

class HeadOfficeCashCounterScreen extends ConsumerStatefulWidget {
  const HeadOfficeCashCounterScreen({super.key});

  @override
  ConsumerState<HeadOfficeCashCounterScreen> createState() =>
      _HeadOfficeCashCounterScreenState();
}

class _HeadOfficeCashCounterScreenState
    extends ConsumerState<HeadOfficeCashCounterScreen> {
  String _searchQuery = '';
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref.read(headOfficeCashCounterProvider.notifier).load();
    });
  }

  @override
  Widget build(BuildContext context) {
    final state = ref.watch(headOfficeCashCounterProvider);
    final isMobile = MediaQuery.of(context).size.width < 768;

    ref.listen<HeadOfficeCashCounterState>(headOfficeCashCounterProvider,
        (_, next) {
      if (next.status == HeadOfficeCashCounterStatus.error &&
        ...
        children: [
            ...
            child: Row(
              children: [
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('Head Office Cash Counter',
                        style: TextStyle(
          ...
          Expanded(
            child: state.isLoading
                ...
                : state.status == HeadOfficeCashCounterStatus.error
                    ? _ErrorView(
                        message:
...
InputDecoration _searchDecor(String hint) => InputDecoration(
      ...
      prefixIcon:
          const TextFieldIcon(AppIcons.search, size: 18, color: Color(0xFF8A8FA3)),
      filled: true,
