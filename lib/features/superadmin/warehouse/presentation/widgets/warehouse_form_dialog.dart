...
class WarehouseFormDialog extends StatefulWidget {
  ...
  final ValueChanged<WarehouseModel> onSave;

  const WarehouseFormDialog({super.key, this.warehouse, required this.onSave});
...
class _WarehouseFormDialogState extends State<WarehouseFormDialog> {
  ...
  void initState() {
    ...
    _name = TextEditingController(text: w?.warehouseName ?? '');
    _address = TextEditingController(text: w?.address ?? '');
    _phone = TextEditingController(text: w?.phoneNumber ?? '');
    _city = TextEditingController(text: w?.city ?? '');
    _status = w?.status ?? 'active';
  }

  @override
  void dispose() {
    _name.dispose(); _address.dispose(); _phone.dispose(); _city.dispose();
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      ...
      title: Text(isEdit ? 'Edit Warehouse' : 'Add Warehouse', style: const TextStyle(fontWeight: FontWeight.w600)),
      content: SizedBox(
        width: 400,
        child: Form(
          key: _formKey,
          child: Column(
            ...
            children: [
              _field(controller: _name, label: 'Warehouse Name *', required: true),
  ...
  Widget _field({required TextEditingController controller, required String label, bool required = false, TextInputType keyboardType = TextInputType.text}) {
