...
class BrandsScreen extends ConsumerStatefulWidget {
  final bool readOnly;
  const BrandsScreen({super.key, this.readOnly = false});
...
class _BrandsScreenState extends ConsumerState<BrandsScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref.read(brandProvider.notifier).loadAll();
    });
  }

  void _showForm({BrandModel? item}) {
    showDialog(
      context: context,
      builder: (_) => BrandFormDialog(
        item: item,
        onSave: (m) => item == null
            ? ref.read(brandProvider.notifier).create(m)
            : ref.read(brandProvider.notifier).update(m),
      ),
    );
  }

  void _confirmDelete(BrandModel item) {
    showDialog(
      context: context,
      builder: (_) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () {
              ref.read(brandProvider.notifier).delete(item.id);
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<BrandState>(brandProvider, (_, next) {
      if (next.status == BrandStatus.error && next.errorMessage != null) {
        ...
        children: [
              ...
              children: [
                Column(
                  ...
                  children: [
                    ...
                    Text('${state.items.length} total brands',
          ...
          Expanded(
            child: state.isLoading
                ...
                : state.status == BrandStatus.error
                    ? Center(
                        child: Column(
                    ...
                    : filtered.isEmpty
                        ? Center(
                            child: Column(
                              mainAxisSize: MainAxisSize.min,
                              children: [
                                AppIcon(AppIcons.brandingWatermarkOutlined,
...
class _TableView extends StatelessWidget {
  ...
  final bool readOnly;
  final Function({BrandModel? item}) onEdit;
  final Function(BrandModel) onDelete;
