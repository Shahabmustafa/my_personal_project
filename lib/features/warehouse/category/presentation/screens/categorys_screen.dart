...
class CategorysScreen extends ConsumerStatefulWidget {
  final bool readOnly;
  const CategorysScreen({super.key, this.readOnly = false});

  @override
  ConsumerState<CategorysScreen> createState() => _CategorysScreenState();
}

class _CategorysScreenState extends ConsumerState<CategorysScreen> {
  String _search = '';
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref.read(categoryProvider.notifier).loadAll();
    });
  }

  void _showForm({CategoryModel? item}) {
    showDialog(
      context: context,
      builder: (_) => CategoryFormDialog(
        item: item,
        onSave: (m) => item == null
            ? ref.read(categoryProvider.notifier).create(m)
            : ref.read(categoryProvider.notifier).update(m),
      ),
    );
  }

  void _confirmDelete(CategoryModel item) {
    showDialog(
      context: context,
      builder: (_) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () {
              ref.read(categoryProvider.notifier).delete(item.id);
            ...
            child: Row(
              children: [
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('Categorys',
                        style: TextStyle(
                            fontSize: 22, fontWeight: FontWeight.bold)),
                    Text('${state.items.length} total categorys',
                    ...
                    : filtered.isEmpty
                        ? Center(
                            child: Column(
                              mainAxisSize: MainAxisSize.min,
                              children: [
                                AppIcon(AppIcons.categoryOutlined,
...
class _TableView extends StatelessWidget {
  ...
  final bool readOnly;
  final Function({CategoryModel? item}) onEdit;
