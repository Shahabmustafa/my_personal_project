...
import '../providers/color_state.dart';
...
class _ColorsScreenState extends ConsumerState<ColorsScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref.read(colorProvider.notifier).loadAll();
    });
  }

  void _showForm({ColorModel? item}) {
    showDialog(
      context: context,
      builder: (_) => ColorFormDialog(
        item: item,
        onSave: (m) => item == null
            ? ref.read(colorProvider.notifier).create(m)
            : ref.read(colorProvider.notifier).update(m),
      ),
    );
  }

  void _confirmDelete(ColorModel item) {
    showDialog(
      context: context,
      builder: (_) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () {
              ref.read(colorProvider.notifier).delete(item.id);
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(colorProvider);

    ref.listen<ColorState>(colorProvider, (_, next) {
      if (next.status == ColorStatus.error && next.errorMessage != null) {
        ...
        children: [
              ...
              children: [
                Column(
                  ...
                  children: [
                    ...
                    Text('${state.items.length} total colors',
          ...
          Expanded(
            child: state.isLoading
                ...
                : state.status == ColorStatus.error
                    ? Center(
                        child: Column(
                    ...
                    : filtered.isEmpty
                        ? Center(
                            child: Column(
                              mainAxisSize: MainAxisSize.min,
                              children: [
                                AppIcon(AppIcons.colorLensOutlined,
