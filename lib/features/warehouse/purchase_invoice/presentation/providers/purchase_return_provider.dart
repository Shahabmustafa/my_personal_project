...
class CashCounterNotifier extends StateNotifier<WarehouseCashCounter?> {
  ...
  CashCounterNotifier(this._repo, this._warehouseId) : super(null) {
    load();
  }
...
class ReturnListNotifier extends StateNotifier<ReturnListState> {
  ...
  ReturnListNotifier(this._repo, this._warehouseId)
      : super(const ReturnListState());

  Future<void> loadReturns() async {
    state = state.copyWith(isLoading: true, error: null);
    try {
      final list = await _repo.getReturns(_warehouseId);
...
FutureProvider<List<Map<String, String>>>(
      ...
      .getInvoiceNumbers(ref.watch(currentWarehouseIdProvider)),
);
...
class PurchaseReturnState {
  ...
  // Purchase return mein bhi purchasePrice use hoti hai - company wapas itna hi deti hai
  double get totalAmount =>
      cartItems.fold(0, (s, i) => s + (i.purchasePrice * i.quantity));
  double get totalDiscount => cartItems.fold(0, (s, i) => s + (i.purchaseDiscountAmount * i.quantity));
  ...
  PurchaseReturnState copyWith({
    ...
    bool clearError = false,
      ...
      PurchaseReturnState(
        ...
        clearInvoice ? null : selectedInvoiceNumber ?? this.selectedInvoiceNumber,
...
class PurchaseReturnNotifier extends StateNotifier<PurchaseReturnState> {
  ...
  PurchaseReturnNotifier(this._repo, this._warehouseId)
