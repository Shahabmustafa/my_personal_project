...
/// Branch ki sab sale returns — manager apne branch ke har return ka
/// accountable hota hai (sale_returns table mein per-manager attribution
/// nahi hoti, branch ka ek hi manager hota hai).
class ManagerReturnsScreen extends ConsumerWidget {
  const ManagerReturnsScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final returnsAsync = ref.watch(branchSaleReturnsProvider);

    return Scaffold(
      backgroundColor: const Color(0xFFF7F8FC),
      body: RefreshIndicator(
        onRefresh: () async {
          ref.invalidate(branchSaleReturnsProvider);
          await ref.read(branchSaleReturnsProvider.future);
        },
