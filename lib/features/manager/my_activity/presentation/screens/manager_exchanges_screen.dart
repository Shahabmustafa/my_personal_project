...
/// Branch ki sab sale exchanges — sale_exchanges table mein bhi manager_id
/// column nahi hai, is liye poori branch history dikhti hai.
class ManagerExchangesScreen extends ConsumerWidget {
  const ManagerExchangesScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final exchangesAsync = ref.watch(branchSaleExchangesProvider);

    return Scaffold(
      backgroundColor: const Color(0xFFF7F8FC),
      body: RefreshIndicator(
        onRefresh: () async {
          ref.invalidate(branchSaleExchangesProvider);
          await ref.read(branchSaleExchangesProvider.future);
        },
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text('${x.differenceLabel} ${x.differenceAmount.abs().toStringAsFixed(0)}',
                    style: TextStyle(
                        fontSize: 14, fontWeight: FontWeight.w700, color: diffColor)),
              ],
            ),
          ),
        );
