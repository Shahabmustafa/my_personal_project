...
/// Logged-in manager ka apna salary/commission record.
final managerProfileProvider = FutureProvider<EmployeeSalaryModel?>((ref) {
  final userId = ref.watch(authProvider).user?.id ?? '';
  if (userId.isEmpty) return Future.value(null);
  return ref.read(employeeSalaryRepositoryProvider).getByUser(userId);
});

/// Pichle mahinon ka salary/commission archive.
final managerSalaryHistoryProvider =
    FutureProvider<List<EmployeeSalaryHistoryModel>>((ref) async {
  final profile = await ref.watch(managerProfileProvider.future);
