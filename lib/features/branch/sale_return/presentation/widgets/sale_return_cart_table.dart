...
/// Sale Invoice ke [SaleCartTable] jaisa hi hai — return cart ke liye,
/// qty par koi stock-cap nahi.
class SaleReturnCartTable extends ConsumerWidget {
  const SaleReturnCartTable({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final items = ref.watch(saleReturnProvider).cartItems;
    ...
    if (items.isEmpty) {
      return Center(
        child: Column(
          ...
          children: [
            AppIcon(AppIcons.assignmentReturnOutlined, size: 64, color: Colors.grey.shade300),
          ...
          Expanded(
            ...
            child: Padding(
              ...
              child: _QtyRowStepper(
                value: item.quantity,
                accentColor: accent,
                onChanged: (v) => notifier.updateItemQuantity(item.branchStockId, v),
              ),
            ),
          ),
...
// ── Qty stepper inside row (koi max cap nahi) ────────────────────────────
