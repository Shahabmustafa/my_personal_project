...
/// [BranchTransferNotifier] instead of the sale invoice cart.
class BranchTransferProductSelector extends ConsumerStatefulWidget {
  const BranchTransferProductSelector({super.key});

  @override
  ConsumerState<BranchTransferProductSelector> createState() =>
      _BranchTransferProductSelectorState();
}

class _BranchTransferProductSelectorState
    extends ConsumerState<BranchTransferProductSelector> {
  ...
  List<_ProductItem> _buildProducts(List<BranchStockModel> all) {
    ...
    for (final s in all.where((s) => s.quantity > 0)) {
      seen.putIfAbsent(
          s.productId,
  ...
  List<BranchStockModel> _sizesFor(List<BranchStockModel> all, String productId) {
    ...
    for (final s in all.where((s) => s.productId == productId && s.quantity > 0)) {
  ...
  List<BranchStockModel> _colorsFor(
    ...
    for (final s in all.where((s) =>
        s.productId == productId && s.sizeId == sizeId && s.quantity > 0)) {
